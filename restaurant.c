...
void savePurchase(char name[], double total) {
...
void displaySpender() {
    ...
    while (fscanf(fp, "%s %.2lf", customers[count].name, &customers[count].total) != EOF) {
        count ++;
    }
    fclose(fp);
...
void searchCustomer(char searchName[]) {
    ...
    while (fscanf(fp, "%s %.2lf", p.name, &p.total) != EOF) {
        if (strcmp(p.name, searchName) == 0) {
            printf("Customer found: %s has total spend of Rp%.2lf\n", p.name, p.total);
            found = 1;
        }
    }
...
int main() {
	...
	double total1, total2, total3, total4, currentTotal = 0;
	char more;
	...
	while (keepOrdering) {
		...
		if (mainMenu == 1) {
			...
			printf("Your total is Rp%.2lf", currentTotal);
